...
{
    float adcVoltage = ADCbits/1024.0f; // converts ADC bits to number between 0-1.
    return adcVoltage;
}
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        if (i == 2)
        {
            ...
            for(x=0; x<numsamples; x++)//calculate average FB_USB
                    ...
                    avgVoltage = avgVoltage/numsamples;
            float calibratedBAT = avgVoltage*BATmultiplier;
...
int main(void)
{
    ...
    while(1)
    {
        ...
        int wholePart = (int)calibratedBAT;
        int decimalPart = (int)((calibratedBAT - wholePart) * 10000);
        printf("calibratedbat %d .%d", wholePart, decimalPart);
                if (batcount<statusledtime){

                if(calibratedBAT>lowbat)
                {
                    GPIO_WriteBit(GPIOC, RED, Bit_SET);


                    if (calibratedBAT>midbat)
                    {


                    GPIO_WriteBit(GPIOC, YLW, Bit_SET);
                    if (calibratedBAT>fullbat)
                    {
