...
int midbat = 3.2;
int fullbat = 3.4;
int statusledtime = 150; // for 10 seconds, set as 294
float calibratedbattery = 3.4;
...
//not sure of the use of this, however crucial for adc
...
// initialise periodic wakeup from standby mode
...
void standbyConfig()
{
    ...
    PWR_AWU_SetWindowValue(25); //sets a time delay of 10 seconds when the microcontroller periodically wakes back up
    PWR_AutoWakeUpCmd(ENABLE);
}

// functions that returns adc value only
...
float getFB_SOLAR()
{
    ...
    calibratedSOLAR = adcVoltage * SOLARmultiplier;
...
float getFB_BAT()
{
    ...
    calibratedbattery = adcVoltage * BATmultiplier;
    return calibratedbattery;
}

float getFB_LED()
{
    float adcVoltage = calculateVoltage(ADCBuffer[3]);
    calibratedLED = adcVoltage * LEDmultiplier;
...
// function that prints the relevent adc value
void printFB_(float calibratedvalue, const char *FB)
{

             int wholePart = (int)calibratedvalue;
             int decimalPart = (int)((calibratedvalue - wholePart) * 10000);
             printf("%s : %d.%04d ", FB, wholePart, decimalPart);
}
...
//initialising serial and clocks
    ...
    while(1)
    {
        ...
        //float calibratedUSB = getFB_USB();
        //float calibratedSOLAR = getFB_SOLAR();
        float calibratedbattery = getFB_BAT();
        //float calibratedLED = getFB_LED();

        printFB_(calibratedbattery,"calibratedbattery");


     // calculate averages of all values across samples number each time main while loops
        for(x=0; x<samples; x++)
        {
            total_bat = total_bat + getFB_BAT();
            total_usb = total_usb + getFB_USB();
            total_solar = total_solar + getFB_SOLAR();
            total_led = total_led + getFB_LED();
        }
        ...
        //printFB_(avg_led,"avg_led");
        printFB_(avg_solar,"avg_solar");
        printFB_(avg_usb,"avg_usb");
     ...
     /*   if (avg_solar > 0.27)
        ...
        //if battery voltage is less than 3.2, enter sleep mode and periodically wake every 10s
        // modetracker takes approx 2 minutes to reach 4000. If the LED mode is 0 fro 2 minutes, the microcontroller goes to sleep.
        if (avg_bat< 3.2 || modetracker > 4000) // use average battery to avoid dodgy connection
        {
                   //when going to sleep, the orange LED shows for 2 seconds

                       GPIO_WriteBit(GPIOC, GPIO_Pin_6, Bit_SET);
                       Delay_Ms(2000);
                       GPIO_WriteBit(GPIOC, GPIO_Pin_6, Bit_RESET);
                       printf("GONE TO SLEEP__");
                     ...
                     //green LED shows for 2 seconds to signal the mcu waking back up
        ...
        //count up to 2 minutes when LEDs are on mode 0
        if (LEDmode == 0){
            modetracker ++;
            printf("modetracker is %d", modetracker);
        }
        else{
            modetracker = 0;
        }
